...
func waitUntil(
    ...
    await eventuallyOnMainActor(timeout: timeout, file: file, line: line, condition)
}
...
func waitUntil(
    ...
    let isInitializationHandoff = !model.isInitialized
    let didSatisfy = await eventuallyOnMainActor(
        timeout: timeout,
        file: file,
        line: line,
        condition
    )
    // Model publication often schedules its owned lifecycle task in the same actor turn.
    // Preserve the historical helper's event-driven handoff without taking ownership of
    // `statusDidChange`, which belongs to the code under test.
    if didSatisfy, isInitializationHandoff, model.isInitialized {
